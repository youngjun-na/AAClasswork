...
def no_apples_for_blair_sub
  ...
  execute(<<-SQL)
    ...
    JOIN
      toys ON cattoys.toy_id = toys.id
    WHERE
      toys.name = 'Apple') AND name != 'Blair'
    ORDER BY
      cats.name;
  SQL
end
...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    JOIN
      toys AS toys2 
    ON
      toys1.price = toys2.price
    WHERE
      toys2.name = 'Shiny Mouse' AND toys1.name != 'Shiny Mouse';
  SQL
end
...
def just_like_orange
  ...
  execute(<<-SQL)
    ...
    JOIN
      cats as cats2
    ON
      cats1.breed = cats2.breed
    WHERE
      cats2.name = 'Orange' AND cats1.name != 'Orange'
    ORDER BY
      cats1.name

...
export class Board {
  constructor(rowSize, colSize, numBombs) {
  ...
  onBoard(pos) {
    return (
      pos[0] >= 0 && pos[0] < this.rowSize &&
        pos[1] >= 0 && pos[1] < this.colSize
    );
  }
  ...
  plantBombs() {
    ...
    while (totalPlantedBombs < this.numBombs) {
      const row = Math.floor(Math.random() * (this.rowSize - 1));
      const col = Math.floor(Math.random() * (this.colSize - 1));
